...
`,r+=`- 吐槽时必须带"喵"。
`,r+=`- 禁止写出人类式的长篇大论。
`),r},h=async(e,t,n,r,i=!1)=>{if(!e)return`Error: No Character`;if(!i&&e.writerPersona&&e.writerPersonaGeneratedAt&&Date.now()-e.writerPersonaGeneratedAt<10080*60*1e3)return e.writerPersona;let a=`你是一位人物心理分析专家和写作教练。我会给你一个虚拟角色的完整档案，以及与他/她互动的用户档案。请你深入理解这个角色，然后告诉我：
...
**字数要求**：总共400-600字。`;try{let t=await fetch(`${n.baseUrl.replace(/\/+$/,``)}/chat/completions`,{method:`POST`,headers:{"Content-Type":`application/json`,Authorization:`Bearer ${n.apiKey}`},body:JSON.stringify({model:n.model,messages:[{role:`user`,content:a}],temperature:.7,max_tokens:6e4})});if(t.ok){let n=(await o(t)).choices[0].message.content.trim(),i=`
...
`.trim();return r(e.id,{writerPersona:i,writerPersonaGeneratedAt:Date.now()}),i}else throw Error(`API Error: ${t.status}`)}catch(t){return console.error(`Deep analysis failed:`,t),p(e)}},g=e=>{let t=(e.impression?.personality_core.observed_traits||[]).find(e=>[`冷漠`,`高冷`,`感性`,`温柔`,`乐天`,`活泼`,`中二`,`电波`].some(t=>e.includes(t)))||`理性`;t.includes(`冷`)&&(t=`冷漠`),(t.includes(`柔`)||t.includes(`感`))&&(t=`感性`),(t.includes(`乐`)||t.includes(`活`))&&(t=`乐天`);let n={冷漠:`
**错误示范（AI机械味）**：
...
`};return n[t]||n.理性},ee=(e,t,n,r,i,a,o,c)=>{let l=s.buildCoreContext(e,t,!0),u=e.writerPersona||p(e),d=g(e),f=m(e),h=n?.protagonists.map(e=>`- ${e.name} (${e.role}): ${e.description}`).join(`
`)||`无`,ee=`
小说：《${n?.title}》
世界观：${n?.worldSetting}
主要角色：
...
2. **正文续写**: 
   ...
   - 节奏: 符合你的性格。
   - 字数: 400-800字。
`,v.push(`"writer": { "content": "正文内容", "technique": "技巧", "mood": "基调" }`)),a.comment){let t=o.slice(-5).filter(t=>t.authorId!==`user`&&t.authorId!==e.id&&(t.role===`writer`||t.type===`story`)).map(e=>({name:c.find(t=>t.id===e.authorId)?.name||`Unknown`,content:e.content.substring(0,100)}));_+=`
3. **吐槽/感想 (带互动)**: 
